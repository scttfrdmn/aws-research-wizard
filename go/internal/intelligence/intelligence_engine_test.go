...
func TestIntelligenceEngine_selectOptimalInstance(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedInstance: "r6i.16xlarge", // Matches actual fallback logic
		},
		{
			name:             "ml_small",
		...
		},
		{
			...
			domain:           "unknown",
			workloadSize:     "large",
			expectedInstance: "c6i.12xlarge", // Matches actual fallback logic
		},
		{
			name:             "unknown_domain_massive",
			domain:           "unknown",
			workloadSize:     "massive",
			expectedInstance: "c6i.24xlarge", // Matches actual fallback logic
		},
		{
			name:             "empty_domain",
			domain:           "",
		...
		},
		{
			...
			expectedInstance: "c6i.2xlarge", // Matches actual default fallback
		},
	}
...
func TestIntelligenceEngine_generateStorageConfiguration(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedType:  "gp3",
			expectedIOPS:  8000, // Genomics profile uses 8000 IOPS
		},
		{
		...
		},
		{
			...
			expectedType:  "gp3", // Still uses gp3, not based on access pattern
			expectedIOPS:  8000,  // Genomics profile uses 8000 IOPS
		},
		{
			name:          "medium_random",
...
func TestIntelligenceEngine_generateNetworkConfiguration(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:              "large_workload",
			workloadSize:      "large",
			expectedEFA:       false, // Genomics domain doesn't enable EFA
			expectedPlacement: true,  // Large workloads enable placement groups
		},
		{
			name:              "medium_workload",
			workloadSize:      "medium",
			expectedEFA:       false,
			expectedPlacement: false, // Only large/massive enable placement groups
		},
		{
			name:              "small_workload",
			workloadSize:      "small",
...
func TestIntelligenceEngine_generateGeneralResourcePlan(t *testing.T) {
	ie := createTestIntelligenceEngine()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dataPattern := &data.DataPattern{
				...
				TotalFiles: 1000,
				FileSizes: data.FileSizeAnalysis{
					MeanSize: 1024 * 1024, // 1MB average
				},
			}
			...
			plan := ie.generateGeneralResourcePlan(dataRec, hints)

			if plan == nil {
				t.Fatal("generateGeneralResourcePlan() returned nil")
			}
			...
			if !plan.SecurityConfiguration.EncryptionAtRest {
				t.Error("Expected encryption at rest to be enabled")
			}

			if !plan.SecurityConfiguration.EncryptionInTransit {
				t.Error("Expected encryption in transit to be enabled")
			}

			// Verify network configuration
			if !plan.NetworkConfiguration.EnhancedNetworking {
				t.Error("Expected enhanced networking to be enabled")
			}
		})
	}
}

func TestIntelligenceEngine_toolsMatch_EdgeCases(t *testing.T) {
	ie := createTestIntelligenceEngine()
	...
	tests := []struct {
		...
		{
			name:     "reverse_containment_match",
			tool1:    "numpy",
			tool2:    "scientific-python-numpy-toolkit",
			expected: true,
		},
		{
			name:     "no_match",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := ie.toolsMatch(tt.tool1, tt.tool2)
			if result != tt.expected {
				t.Errorf("toolsMatch(%q, %q) = %v, want %v", tt.tool1, tt.tool2, result, tt.expected)
			}
		})
	}
}

func TestIntelligenceEngine_selectOptimalInstance_FallbackCases(t *testing.T) {
	ie := createTestIntelligenceEngine()
	...
	tests := []struct {
		...
		{
			name:             "machine_learning_massive_fallback",
			domain:           "machine_learning",
			workloadSize:     "massive",
			expectedInstance: "c6i.24xlarge", // Fallback to default massive instance
		},
		{
			name:             "genomics_fallback_to_profile_logic",
			domain:           "genomics",
			workloadSize:     "unknown_size",
			expectedInstance: "c6i.2xlarge", // Falls back to base instance
		},
		{
			name:             "climate_domain_with_nonexistent_pack",
			domain:           "climate",
			workloadSize:     "medium",
			expectedInstance: "c6i.4xlarge", // Falls back to profile logic
		},
		{
			name:             "machine_learning_small_gpu_selection",
			domain:           "machine_learning",
			workloadSize:     "small",
			expectedInstance: "g5.xlarge", // ML gets GPU instances
		},
		{
			name:             "machine_learning_medium_gpu_selection",
			domain:           "machine_learning",
			workloadSize:     "medium",
			expectedInstance: "g5.4xlarge", // ML gets GPU instances
		},
		{
			name:             "machine_learning_large_gpu_selection",
			domain:           "machine_learning",
			workloadSize:     "large",
			expectedInstance: "p4d.24xlarge", // ML gets high-end GPU for large
		},
	}
...
func TestIntelligenceEngine_generateAlternativeInstances_DomainSpecific(t *testing.T) {
	ie := createTestIntelligenceEngine()
	...
	tests := []struct {
		...
		{
			name:         "machine_learning_includes_gpu",
			domain:       "machine_learning",
			workloadSize: "medium",
			expectMLGPU:  true,
		},
		{
			name:         "genomics_no_gpu",
			domain:       "genomics",
			workloadSize: "medium",
			expectMLGPU:  false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			profile := &data.ResearchDomainProfile{Name: tt.domain}
			alternatives := ie.generateAlternativeInstances(profile, tt.workloadSize)

			if len(alternatives) == 0 {
				t.Error("Expected at least one alternative instance")
			}

			// Check if GPU instances are included for ML domain
			hasGPU := false
			for _, instance := range alternatives {
				if strings.Contains(instance, "g5") || strings.Contains(instance, "p4d") {
					hasGPU = true
					break
				}
			}

			if tt.expectMLGPU && !hasGPU {
				t.Error("Expected GPU instances for machine learning domain")
			}
			if !tt.expectMLGPU && hasGPU && tt.domain != "machine_learning" {
				t.Error("Did not expect GPU instances for non-ML domain")
			}
		})
	}
}

func TestIntelligenceEngine_generateStorageConfiguration_EdgeCases(t *testing.T) {
	ie := createTestIntelligenceEngine()
	...
	tests := []struct {
		...
		{
			name:         "astronomy_domain_large_storage",
			domain:       "astronomy",
			totalSizeGB:  1000.0,
			expectedType: "gp3",
		},
		{
			name:         "machine_learning_high_iops",
			domain:       "machine_learning",
			totalSizeGB:  500.0,
			expectedType: "gp3",
		},
		{
			name:         "unknown_domain_default",
			domain:       "unknown",
			totalSizeGB:  100.0,
			expectedType: "gp3",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if config.PrimaryStorage.Type != tt.expectedType {
				t.Errorf("Expected storage type %s, got %s", tt.expectedType, config.PrimaryStorage.Type)
			}

			// Verify storage size includes buffer
			expectedMinSize := int(tt.totalSizeGB) + 200
			if config.PrimaryStorage.SizeGB < expectedMinSize {
				t.Errorf("Expected storage size at least %d, got %d", expectedMinSize, config.PrimaryStorage.SizeGB)
			}
		})
	}
}

func TestIntelligenceEngine_generateNetworkConfiguration_DomainSpecific(t *testing.T) {
	ie := createTestIntelligenceEngine()
	...
	tests := []struct {
		...
		{
			name:        "climate_enables_efa",
			domain:      "climate",
			workload:    "medium",
			expectedEFA: true,
		},
		{
			name:        "machine_learning_enables_efa",
			domain:      "machine_learning",
			workload:    "small",
			expectedEFA: true,
		},
		{
			name:        "genomics_no_efa",
			domain:      "genomics",
			workload:    "large",
			expectedEFA: false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			profile := &data.ResearchDomainProfile{Name: tt.domain}
			config := ie.generateNetworkConfiguration(profile, tt.workload)

			if config.EFAEnabled != tt.expectedEFA {
				t.Errorf("Expected EFA %v for %s domain, got %v", tt.expectedEFA, tt.domain, config.EFAEnabled)
			}
		})
	}
}
